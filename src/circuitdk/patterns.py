...
class Interface(Construct):
    def __init__(
        ...
        *,
        ...
        super().__init__(scope, construct_id)
        if not pins:
            raise ValueError("an interface must expose at least one role")
        self.pins = dict(pins)

    def pin(self, role: str) -> Pin:
        try:
            return self.pins[role]
        except KeyError as error:
            raise KeyError(f"interface {self.path} has no role {role!r}") from error

    def connect(self, other: Interface, *, roles: dict[str, str] | None = None) -> None:
        mapping = roles or {role: role for role in self.pins.keys() & other.pins.keys()}
        if not mapping:
            raise ValueError(f"interfaces {self.path} and {other.path} have no common roles")
        for own_role, other_role in mapping.items():
            self.circuit.connect(self.pin(own_role), other.pin(other_role))
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            scope,
            construct_id,
            pins={"sck": sck, "mosi": mosi, "miso": miso, "chip_select": chip_select},
        )
